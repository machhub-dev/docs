...
:::note
These raw operators are canonical. Some service templates accept named aliases
(`'eq'`, `'ne'`, `'gt'`, `'contains'`, …) and translate them internally, and a few use
`skip` as a synonym for `offset`. Prefer the raw operators and `offset`/`limit` shown
here.
:::
