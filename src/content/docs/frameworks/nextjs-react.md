...
calls `sdk.Initialize()` in a `useEffect`. The only difference between zero-config and
...
For production, pass connection details read from `NEXT_PUBLIC_*` environment
variables:
...
The SDK initializes client-side, so its config must use the `NEXT_PUBLIC_` prefix —
only those variables are bundled into the browser. Never expose a developer key with
`NEXT_PUBLIC_`; keep secrets unprefixed and use them only in Server Components,
