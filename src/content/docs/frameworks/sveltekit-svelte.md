...
description: Integrate the MACHHUB SDK with SvelteKit and Svelte 5 using an sdk.service, runes-based collection stores, onMount initialization in +layout.svelte, and hooks.server.ts route guards. The SDK works on both client and server.
sidebar:
  order: 5
---

This guide shows the idiomatic way to use [`@machhub-dev/sdk-ts`](/sdk/initialization/)
in a SvelteKit / Svelte 5 application: an `sdk.service.ts`, initialized in
`+layout.svelte` `onMount`, a runes-based collection store (`createCollectionStore`),
and a `hooks.server.ts` route guard.

The SDK runs in **both client and server** contexts. This guide initializes it on the
client (via `onMount`) for reactive UI, and you can also use it server-side in
`hooks.server.ts` or `+page.server.ts`. See the
[Framework Guides Overview](/frameworks/overview/) and the [SSR notes](#6-ssr-notes).
...
browser. No-config vs manual is just whether you pass a config to `sdk.Initialize()`.
...
Read values from `$env/static/public` and pass them to `Initialize` **only** when you
self-host the app, target a different server/domain, or want to hardcode the
connection. This is the `sdk.service.manual.ts` variant:
...
This guide renders the SDK-backed tree on the client. Pair the layout with a
`+layout.ts` that sets `ssr = false` for that tree and kicks off initialization behind
a `browser` check (the SDK also runs server-side — this is a setup choice, not a
requirement):
...
export const ssr = false; // this guide renders this tree on the client
...
The SDK runs in **both client and server** contexts. This guide initializes it on the
client for reactive stores, which is why the SDK-backed tree uses `ssr = false`:

- Setting **`export const ssr = false`** keeps the client-initialized store tree from
  rendering on the server. If a client `load` reaches the SDK, guard it with
  `import { browser } from '$app/environment'` before calling `initializeSDK()` /
  `getSDK()`.
- For server-side data and route protection, use a server context like
  `hooks.server.ts` or `+page.server.ts`. The SDK works there too — initialize it in
  that context — or call the [REST API](/api/overview/) directly.

Use `hooks.server.ts` for server-side route guards (it runs on every request, on the
server). The example below checks an auth cookie and `redirect`s; you could also
initialize the SDK here to validate the session:
...
For server-side mutations, a form action in `+page.server.ts` can use the SDK or call
the REST API directly. This example uses `fetch`:
...
export const actions = {
  create: async ({ request }) => {
    ...
    // Server-side mutation via the REST API (the SDK works here too)
