...
| `sql` | The result of a domain-scoped SurrealDB query | `query` |
...
- Each domain has its own `venv` (Python) and `node_modules` (TypeScript), and
  communicates with the API over per-domain [NATS](/concepts/architecture/) subjects.
