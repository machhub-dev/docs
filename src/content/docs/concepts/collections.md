...
- **name** — unique within the domain. Internally a collection is namespaced as
  `<domainID>.<name>` so two domains can both have a `products` collection.
...
You can define collections three ways:
...
3. **Over the API** — `POST /api/collection`. See the [API Reference](/api/management/).
