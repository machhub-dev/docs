...
| Method | Path | Purpose |
| --- | --- | --- |
| `POST` | `/machhub/function/execute` | Execute a registered function by type and name |

```http
POST /machhub/function/execute
Content-Type: application/json
Domain: domains:acme
...
Serverless **Processes** are invoked through a separate surface
(`POST /machhub/processes/execute` and `POST /process/<slug>` for HTTP-triggered
processes), described in [Invoking a Process](/processes/invoking/). Do not confuse
...
`acme.products` — the prefix is `<domainID>.<table>`.

```mermaid
flowchart LR
  Req["POST /machhub/products\nDomain: domains:acme"] --> Pfx["prefix table\nacme.products"]
  Pfx --> Store[("SurrealDB store")]
```
...
- API: [Authentication](/api/authentication/),
  [Management API](/api/management/), [Conventions & errors](/api/errors/)
